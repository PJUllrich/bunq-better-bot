...
class BudgetLogic:
    ...
    def create(name, iban, limit, duration):
        b = Budget(name, iban, limit, duration)
        Budget.add_budget(b)

    @staticmethod
    def get_updates():
        """
        Calculates the total amount of the outgoing expenses (except for payments to own
        accounts) in a specified time range (Default: for the last day)
